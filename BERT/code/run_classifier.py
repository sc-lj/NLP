...
flags.DEFINE_integer("max_seq_length", 128,
                     "The maximum total input sequence length after WordPiece tokenization. Sequences longer than this "
...
tf.flags.DEFINE_string("tpu_name", None,
                       "The Cloud TPU to use for training. This should be either the name used when creating the Cloud TPU, "
...
def convert_single_example(ex_index, example, label_list, max_seq_length,tokenizer):
  """将单个 `InputExample` 转换成单个`InputFeatures`.
  如果传入的example是`PaddingInputExample`类，直接转换成`InputFeatures`，否则就要对example进行处理
  Args:
      ex_index: 样本index
      example: 一个`InputExample`的实例
      label_list: `InputExample`对应的标签
      max_seq_length: 一个序列的最大长度
      tokenizer: 一个`FullTokenizer`类
  """
  ...
  if tokens_b:
    # 将`tokens_a` 和`tokens_b` 的总长度缩减到指定的长度
  ...
  # "type_ids"用来表示是否是第一个句子或第二个句子，在预训练期间学习“type = 0”和“type = 1”的嵌入向量，
  # 并将其添加到字体嵌入向量（和位置向量）。 这不是严格*必需的，因为[SEP]标记明确地区分序列，但它使模型更容易学习序列的概念。

  # 对于分类任务，第一个向量（对应于[CLS]）用作“句子向量”。 请注意，这是唯一有意义的，因为整个模型都经过微调。
  tokens = []
  segment_ids = []
  tokens.append("[CLS]")
  ...
  # 对真实的tokens mask为1，对于填充的tokens mask为0，只有真实的tokens被添加
...
def file_based_convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, output_file):
  """将 `InputExample`的集合转换成一个 TFRecord 文件."""
...
def file_based_input_fn_builder(input_file, seq_length, is_training,drop_remainder):
  ...
  def _decode_record(record, name_to_features):
    """将一条record解码为TensorFlow示例。"""
    example = tf.parse_single_example(record, name_to_features)

    # tf.Example 只支持tf.int64,但是TPU只支持 tf.int32。
  ...
  def input_fn(params):
    ...
    #  对于训练过程，需要大量平行的阅读和shuffle；
    # 对于验证过程，不需要shuffle，平行阅读也不需要
    d = tf.data.TFRecordDataset(input_file)
    if is_training:
      d = d.repeat()
      d = d.shuffle(buffer_size=100)

    # func横跨dataset的batch_size个连续元素，然后将它们组合成一个batch
    # drop_remainder 表示是否应丢弃最后一个batch，以防其大小小于所需值; 默认行为是不删除较小的batch。
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, labels, num_labels, use_one_hot_embeddings):
  ...
  Args:
      bert_config: 参数配置文件
      is_training: bool，
      input_ids: tensor，输入文本的ids
      input_mask: tensor，输入文本的mask
      segment_ids:
      labels:
      num_labels:
      use_one_hot_embeddings:是否使用TPU or GPU/CPU.
  """
  ...
  # 如果想使用token级别的输出，用model.get_sequence_output()代替
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,num_train_steps, num_warmup_steps, use_tpu,

...
tf.flags.DEFINE_string('export_path','./data','path to data')

config_file = open(os.path.join('data', "config"), 'r')
config = json.loads(config_file.read())
config_file.close()

tf.flags.DEFINE_integer('max_length', config['fixlen'], 'maximum of number of words in one sentence')
tf.flags.DEFINE_integer('pos_num', config['maxlen'] * 2 + 1, 'number of position embedding vectors')
tf.flags.DEFINE_integer('num_classes', len(config['relation2id']),'maximum of relations')

tf.flags.DEFINE_integer('hidden_size',230,'hidden feature size')
tf.flags.DEFINE_integer('pos_size',5,'position embedding size')
tf.flags.DEFINE_integer('word_size', 50, 'word embedding size')

tf.flags.DEFINE_integer('max_epoch',60,'maximum of training epochs')
tf.flags.DEFINE_integer('batch_size',160,'entity numbers used each training time')
tf.flags.DEFINE_float('learning_rate',0.5,'entity numbers used each training time')
tf.flags.DEFINE_float('weight_decay',0.00001,'weight_decay')
tf.flags.DEFINE_float('drop_prob',0.5,'dropout rate')
...
tf.flags.DEFINE_string('test_result_dir', './test_result', 'path to store the test results')
tf.flags.DEFINE_integer('save_epoch', 2, 'save the checkpoint after how many epoches')

tf.flags.DEFINE_string('model_name', 'pcnn_att', 'model\'s name')
tf.flags.DEFINE_string('pretrain_model', 'None', 'pretrain model')
